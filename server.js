...
var app = express();
var mongourl = 'mongodb://g1166005:Kappa819@ds054289.mlab.com:54289/s1166005';
var ensureLogin = require('connect-ensure-login')
var SECRETKEY1 = 'A34EF53BE6F7BE38AD3EBB1EBF6D4';
var SECRETKEY2 = '936CA149EAC5F1F536F558E7E9486';
...
passport.deserializeUser(function(id, callback) {
    MongoClient.connect(mongourl, function(err, db) {
        db.collection('login').findOne({username:id}, function (err, user) {
            if (err) {
                return callback(err);
            }
            callback(null, user);
...
passport.use('local',new LocalStrategy(
    function(username, password, done) {
        MongoClient.connect(mongourl, function(err, db) {
            db.collection('login').findOne({username: username},function(err,user) {
                ...
                if (user===null) {
                    return done(null, false, {message: 'Please input a username.'});
                }
...
app.get('/display',ensureLogin.ensureLoggedIn(),function(req,res) {
    ...
    MongoClient.connect(mongourl, function(err, db) {
        ...
        findOne({_id: ObjectId(queryAsObject._id)},function(err,doc) {
            assert.equal(err,null);
            checkRated(db,queryAsObject._id,req.user.username,function(callback){
                db.close();
                if(callback!==false){
                    res.render('display',{restaurant:doc,user:req.user.username,rated:true,score:callback.grades[0].score,backToIndex:backToIndex});
                }else{
                    res.render('display',{restaurant:doc,user:req.user.username,rated:false,score:false,backToIndex:backToIndex});
                }
            });
        });
    });
...
app.get('/edit',ensureLogin.ensureLoggedIn(),function(req,res) {
    var parsedURL = url.parse(req.url,true);
    var queryAsObject = parsedURL.query;
    if(queryAsObject.creator !== req.user.username){
...
function userRegister(db,username,password,callback) {
    db.collection('login').findOne({username: username},function(err,doc) {
        assert.equal(err,null);
        if(doc===null){
...
function checkRated(db,objId,username,callback) {
    db.collection('restaurants').findOne({
        "_id": ObjectId(objId),
        grades: {$elemMatch: {username: username}}
    },function(err, doc) {
        if (doc == null) {
            callback(false);
        } else {
